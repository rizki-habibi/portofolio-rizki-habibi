...
import { Search, Monitor, Radio, Crosshair } from 'lucide-react'
  ...
  return (
    <section id="keahlian" className="relative py-28 lg:py-36 bg-hitam overflow-hidden">
      <div className="absolute top-8 left-8 font-judul text-white/[0.03] select-none pointer-events-none"
        style={{ fontSize: 'clamp(6rem, 18vw, 14rem)', lineHeight: 1 }}>03</div>
      ...
      <div ref={ref} className="max-w-7xl mx-auto px-6 lg:px-12">
        ...
        >
          ...
          <h2 className="font-judul text-white" style={{ fontSize: 'clamp(3rem, 7vw, 5.5rem)' }}>KEAHLIAN UNIK</h2>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3 mb-8">
                  <Ikon size={22} className="text-white/60" />
                  <div>
                    <h3 className="font-judul text-white text-xl tracking-wide uppercase">{grup.kategori}</h3>
                    <div className="w-8 h-px bg-white/30 mt-1" />
                  </div>
                </div>
                <div className="space-y-6">
                  {grup.daftar.map((item, ii) => (
                    <BarKeahlian key={item.nama} nama={item.nama} nilai={item.nilai}
                      terlihat={inView} delay={gi * 0.15 + ii * 0.1} />
                  ))}
                </div>
              </motion.div>
            )
        ...
        >
          <p className="font-mono text-xs text-white/30 uppercase tracking-widest mb-6">Kemampuan Tambahan</p>
